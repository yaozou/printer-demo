...
public class InventoryListOfUnarrivedGoodsPrinter extends BasePrinter {
    private InventoryListDto dto;

    public InventoryListOfUnarrivedGoodsPrinter(InventoryListDto dto){
        this.dto = dto;
    }
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        switch (pageIndex) {
            case 0:
                ...
                float start = 0;
                //标题，固定不变
                g2.drawString("未到货入库清单", (float) 60, start += 15);

                font = new Font("新宋体", Font.PLAIN, 12);
                g2.drawString("", (float) 10, start += 10);
                g2.setFont(font);
                int n = 1;
                for (int i =0;i<dto.getList().size();i++){
                    BaseInventoryListInfo info = dto.getList().get(i);
                    n = DrawUtils.drawStringLn(g2,(i+1)+"、"+info.getArea()+" "+info.getOrderNo(), 15, (int)(start += n*16),160);
                }
